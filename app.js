...
async function initTenderPage() {
  ...
  function renderView(rows) {
    ...
    const siteCasted=dynSites.map(s=>rows.filter(r=>r['Site Name']===s).reduce((acc,r)=>acc+(r['Total Qty Casted']||0),0));
    if (dynSites.length) Charts.polar('tender-radar',dynSites,siteQty,{unit:' poles'});

    // Half-circle auto meters (one per site)
    const gaugesEl=document.getElementById('tender-gauges');
    if (gaugesEl && dynSites.length) {
      gaugesEl.innerHTML=dynSites.map((site,i)=>{
        const total=siteQty[i]||1;
        const casted=siteCasted[i]||0;
        const pct=Math.min(100,casted/total*100);
        const col=pct>=75?C.green:pct>=50?C.primary:pct>=25?C.amber:C.red;
        // SVG half-circle gauge: viewBox 0 0 120 68
        // Arc from 180deg to 0deg (left to right), radius=50, cx=60,cy=60
        const r=50, cx=60, cy=60;
        const startX=cx-r, startY=cy; // 180deg
        ...
        const fillArc=pct>0?`M ${cx-r} ${cy} A ${r} ${r} 0 ${pct>50?1:0} 1 ${ex} ${ey}`:'';
        return `<div style="text-align:center;min-width:140px">
          <svg viewBox="0 0 120 68" width="140" height="80" style="overflow:visible">
            <defs>
              <linearGradient id="gmeter-${i}" x1="0%" y1="0%" x2="100%" y2="0%">
                <stop offset="0%" stop-color="${col}" stop-opacity="0.4"/>
                <stop offset="100%" stop-color="${col}" stop-opacity="1"/>
              </linearGradient>
            </defs>
            <!-- Track -->
            <path d="${bgArc}" fill="none" stroke="rgba(255,255,255,0.07)" stroke-width="10" stroke-linecap="round"/>
            <!-- Fill -->
            ${fillArc?`<path d="${fillArc}" fill="none" stroke="url(#gmeter-${i})" stroke-width="10" stroke-linecap="round"/>` :''}
            <!-- Needle tip dot -->
            ${fillArc?`<circle cx="${ex}" cy="${ey}" r="4" fill="${col}" style="filter:drop-shadow(0 0 4px ${col})"/>` :''}
            <!-- Center pct text -->
            <text x="60" y="59" text-anchor="middle" font-size="13" font-weight="700" fill="${col}" font-family="monospace">${pct.toFixed(0)}%</text>
            <!-- Min/Max ticks -->
            <text x="8" y="68" text-anchor="middle" font-size="7" fill="rgba(255,255,255,.35)">0</text>
            <text x="112" y="68" text-anchor="middle" font-size="7" fill="rgba(255,255,255,.35)">${(total/1000>=1?(total/1000).toFixed(0)+'K':total)}</text>
          </svg>
          <div style="font-size:.68rem;font-weight:600;color:rgba(255,255,255,.75);margin-top:-.3rem;letter-spacing:.3px">${site}</div>
          <div style="font-size:.62rem;color:rgba(255,255,255,.4);margin-top:.15rem">${casted.toLocaleString()} / ${total.toLocaleString()} poles</div>
        </div>`;
      }).join('');
    }
    ...
    rows._renderer=r=>{
      ...
      // Expire Date (sheet column L) & age
      const expDate=r['Expire Date']||r['Expiry Date']||r['Contract Expire Date']||null;
      const expDisplay=expDate?fmtDate(expDate):'—';
      let expAgeTxt='—', expAgeCls='text-muted';
      if (expDate) {
        const today=new Date(); today.setHours(0,0,0,0);
        const ed=expDate instanceof Date?new Date(expDate):new Date(expDate);
        ed.setHours(0,0,0,0);
        const diff=Math.floor((ed-today)/86400000);
        if (diff<0)      { expAgeTxt=`${Math.abs(diff)}d ago`; expAgeCls='text-red'; }
        else if (diff===0){ expAgeTxt='Today';                  expAgeCls='text-amber'; }
        else if (diff<=30){ expAgeTxt=`${diff}d left`;          expAgeCls='text-amber'; }
        else              { expAgeTxt=`${diff}d left`;          expAgeCls='text-green'; }
      }
      ...
      return `<tr>
        ...
        <td class="num ${expAgeCls}">${expAgeTxt}</td>
        ...
        <div class="expand-content">
          ...
          <div class="expand-field"><div class="expand-field-label">Expire Date</div><div class="expand-field-val ${expAgeCls}">${expDisplay} ${expAgeTxt!=='—'?'('+expAgeTxt+')':''}</div></div>
          ${isOng?`
