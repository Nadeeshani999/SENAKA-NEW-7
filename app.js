...
async function initTenderPage() {
  ...
  function renderView(rows) {
        ...
        gaugesEl.innerHTML=ongoingForGauges.map((r,i)=>{
          ...
          return `<div style="text-align:center;min-width:148px;${isExpired?'animation:expiredPulse 1.2s ease-in-out infinite;':''}" title="${r['Tender No']||''} · ${poleType} · Limit: ${limitDays}d">
            <svg viewBox="0 0 120 72" width="148" height="88" style="overflow:visible">
              ...
              <!-- Needle dot -->
              ${fillArc?`<circle cx="${ex.toFixed(2)}" cy="${ey.toFixed(2)}" r="${isExpired?5.5:4}" fill="${col}" style="filter:drop-shadow(0 0 ${isExpired?6:3}px ${col})"/>`:''}
              <!-- Age value center -->
              <text x="60" y="55" text-anchor="middle" font-size="15" font-weight="800" fill="${col}" font-family="monospace">${Math.round(ageDays)}d</text>
              <text x="60" y="66" text-anchor="middle" font-size="7.5" fill="rgba(255,255,255,.4)" font-family="sans-serif">of ${limitDays}d limit</text>
              <!-- 0 / limit labels -->
    ...
    rows._renderer=r=>{
      ...
      // Inline mini half-circle badge for the table
      ...
      const miniFill=miniPct>0?`<path d="M ${mCx-miniR} ${mCy} A ${miniR} ${miniR} 0 ${miniPct>50?1:0} 1 ${mEx.toFixed(1)} ${mEy.toFixed(1)}" fill="none" stroke="${miniCol}" stroke-width="3.5" stroke-linecap="round"/>`:'';
      const miniSvg=`<svg viewBox="0 0 32 18" width="36" height="22" style="vertical-align:middle;margin-right:4px">
        <path d="M ${mCx-miniR} ${mCy} A ${miniR} ${miniR} 0 0 1 ${mCx+miniR} ${mCy}" fill="none" stroke="rgba(255,255,255,0.08)" stroke-width="3.5" stroke-linecap="round"/>
        ${miniFill}
        ${miniPct>0?`<circle cx="${mEx.toFixed(1)}" cy="${mEy.toFixed(1)}" r="2.5" fill="${miniCol}"/>` :''}
      </svg>`;
